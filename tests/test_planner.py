...
def test_current_query_routes_to_web():
    profile=profile_query('What is the latest status today?'); assert profile.intent==Intent.CURRENT and profile.freshness_required
...
def test_comparison_is_adaptive():
    ...
    assert plan.rerank and plan.profile.intent==Intent.COMPARISON

def test_short_question_is_fast():
    plan=build_plan('What is RAG?'); assert plan.profile.complexity==Complexity.LOW
